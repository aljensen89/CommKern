...
\end{equation}
However, in many cases a ground truth labelling does not exist, such as brain connectivity. In this case, a two-way matching can be performed, where the matching is performed twice, each time with one of the clusterings being the ground truth and the results combined using a harmonic mean to produce a final matching measure.
...
\end{equation}
where $\mathbf{\beta}$ is a $q \times 1$ vector of regression coefficients, $h\left(\bsmathbf_i\right)$ is an unknown, centered and smooth function, and $e_i$ is an independent and identically distributed error term following $N\left(0,\sigma^2\right)$.
