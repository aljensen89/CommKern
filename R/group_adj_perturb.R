...
#' 
#' This function takes the output from the \code{\link{group_network_perturb}} function, which is a list of
#' dataframes summarizing each simulated network, and creates an array of adjacency matrices. These adjacency
#' matrices can then be used as input to any community detection algorithm (such as \code{link\{hier_mult_spin}}).
#' 
...
#' 
#' @seealso \code{\link{group_network_perturb}}
#' 
#' @export
#' 
group_adj_perturb <- function(group_network_list,n_nets,n_nodes){
  ...
  for (j in 1:n_nets){
    ...
    node_names <- union(group_network_list[[j]]$Node_A,group_network_list[[j]]$Node_B)
    mat <- matrix(0,nrow=n_nodes,ncol=n_nodes,dimnames=list(node_names,node_names))
    mat[as.matrix(group_network_list[[j]][,c(1,2)])] <- group_network_list[[j]][,wgt_col_name]
    mat[as.matrix(group_network_list[[j]][,c(2,1)])] <- group_network_list[[j]][,wgt_col_name]
