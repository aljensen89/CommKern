...
#' 
#' This is an ancillary function that calculates the Rand z-score between two partitions, which is
#' used in the consensus simularity function
#' 
#'The function returns a z-score, which is used in the calculation of the consensus paritition
#' 
...
zrand <- function(part1,part2){
  ...
  #Calculating the Rand and adjusted Rand indices
  ##Rand index
  ...
  #The adjusted coefficient is calculated by subtracting the expected value and rescale the result
  #by the difference between the maximum allowed value and the mean value
  ...
  var <- M/16 - (4*srow-2*M)^2*(4*scol-2*M)^2/(256*M^2) + C1*C2/(16*N*(N-1)*(N-2)) +
    ((4*srow-2*M)^2-4*C1-4*M)*((4*scol-2*M)^2-4*C2-4*M)/(64*N*(N-1)*(N-2)*(N-3))
