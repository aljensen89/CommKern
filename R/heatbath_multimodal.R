heatbath_multimodal <- function(gamma,alpha,temp,max_sweeps){
  ...
  current_communities <- rep(NA,length(network$vertexes$node_id))
  current_hamiltonian <- computeUserGuidedModularity(modularity_matrix,guidance_matrix,
  ...
  while(sweep<max_sweeps){
    ...
    for(spin in 1:q){ #all possible new spins
      if(spin!=old_spin){ #except the old one
        ...
        new_hamiltonian <- computeUserGuidedModularity(modularity_matrix,guidance_matrix,
