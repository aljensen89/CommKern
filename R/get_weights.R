...
#' 
#' @param network_df a dataframe containing information about network nodes, their community assignment, and all node dyads,
#' coming from \code{\link{simnet_df_perturb}}
...
get_weights <- function(network_df,wcr,bcr,bfcr=NA,fuzzy_comms=NA){
  ...
  # the community membership of both nodes
  for(i in seq_along(weights)){
    ...
    if(all(!is.na(bfcr)) & all(!is.na(fuzzy_comms)) &
