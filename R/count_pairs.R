...
count_pairs <- function(a,b,order){
  ...
  #Count pairs
  for (k in 2:n){
    if(name_a[current_position]==a[order[k]+1] & name_b[current_position]==b[order[k]+1]){
  ...
  #Output as list
  ListOut <- list(pair_nb=number_pair,pair_a=name_a,pair_b=name_b,
                  a_nb=count1[count1>0],b_nb=count2[count2>0])
  return(ListOut)
}
