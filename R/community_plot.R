...
#' 
#' @param comm_data is a long data frame with three columns: node_id, layer, and
#' comm; this can be created from the \code{link{comm_layers_tree}} object using
#' the \code{\link[tidyr]{gather}} function.
#' @param node_data is a melted version of the network adjacency matrix, using
#' the reshape2::melt() function
#' 
...
community_plot <- function(comm_data, node_data) {
    ...
    layers <- list(l1 = subset(comm_data, comm_data$layer == "layer_1"), l2 = subset(comm_data,
        comm_data$layer == "layer_2"), l3 = subset(comm_data, comm_data$layer ==
        "layer_3"))

    comms <- lapply(layers, function(x) {
        unique(x$comm2)
    })
    n_comms <- lapply(comms, length)
    comm_colors <- mapply(function(n, s) {
    ...
    comm_plot <- 
      ...
      ggplot2::geom_tile(data = layers[[1]],
                         mapping = ggplot2::aes_string(fill = "comm1"),
                         color = "black") +
