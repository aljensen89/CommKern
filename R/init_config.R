...
#' 
#' @param x a \code{hms_network} object
...
#'   
#' @export
init_config <- function(x, spin) {
  UseMethod("init_config")
}

#' @export
init_config.hms_network <- function(x, spin) {
  s <- 0
  sum_weight <- 0
  
  for (i in 1:length(x$net$vertexes$node_id)){
